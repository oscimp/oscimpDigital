...
\hspace{-1cm}\includegraphics{./block_design.pdf}
\caption{Objective of the tutorial (top) and block design (bottom) including the processor, the reset 
signals, and ADC and a DAC, as well as the clock generation block for the latters.}
...
\section{Inserting new blocks in Vivado}
...
be used lated are not added, such as the {\em axi interconnect} and the
{\em Processor System Reset}. The latter block us however mandatory in the current case
since it handle reset signals. Hence, having again hit {\tt CTRL + i}, select 
{\em Processor System Reset} (search keyword {\tt reset}).

\section{Connecting blocks to the FPGA pins}

The three blocks describing the ADC, DAC and internal signals must be connected
...
and right-mouse click or using the shortcut {\tt CTRL + t}.
...
\begin{itemize}
\item {\tt data\_a} of the ltc2145 is connected to {\tt dataA} of {\tt ad9767}
\item {\tt data\_b} of the ltc2145 is connected to {\tt dataB} of {\tt ad9767}
\end{itemize}
