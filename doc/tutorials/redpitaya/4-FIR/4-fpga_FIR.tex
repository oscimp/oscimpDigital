% ATTENTION seule la voie 2 est FIR, la voie 1 est identit'e
...
This file does need to be manually written since the tool {\tt make xml} in the
{\tt design} directory will parse the generated Vivado output and generate the appropriate
XML file in the project directory.
...
Rather than use a ``simple'' $|sinc()|$ transfer from setting a few taps to 1 and others to
0, a FIR design is obtained with \verb~round((firls(31,[0 .04 .08 1],[1 1 0 0]))*64)~
and \verb~round((firls(31,[0 .02 .04 1],[1 1 0 0]))*64)~ whose output is observed on a network
analyzer and plotted with GNUPlot's (Fig. \ref{gnuplot})

{\footnotesize
\begin{verbatim}
pl 'FIR_S12_p02_p04.csv' u ($1/1e6):2,'FIR_S12_p04_p08.csv' u ($1/1e6):2
set yrange [-60:15]
set xlabel 'freq (MHz)'
set ylabel 'S12 (dB)'
set grid 
...
The responses of the filters is simulated with the Fourier transform of the taps:
...
\noindent and match nicely the experimental observations (Fig. \ref{gnuplot}).

\begin{figure}[h!tb]
% \includegraphics[angle=-90,width=\linewidth]{FIR_S12.pdf}
\begin{center}
\vspace{-0.5cm}
\includegraphics[width=.8\linewidth]{simu_exp}

\includegraphics[width=.7\linewidth]{IMG_20251202_155143_491small.jpg}
\end{center}
\caption{Characterization of the digital FIR filter using a network analyzer. Top:
comparison of simulated response (top) and measured transfer function (bottom). Bottom:
experimental setup.}
\label{gnuplot}
\end{figure}

Notice the display of the S$_{12}$ scattering coefficient since the signal
is output on port 2 of the network analyzer and input on port 1 (Fig. \ref{gnuplot}), bottom
