...
Double click on the FIR block and update the Data Width ({\tt Data In Size}) to 14~bits. The input
...
significant bits to send 14-bit wide words (as expected from the ADC datastream) to the PS.
...
with the name than the IP in the {\tt fpga\_driver} directory.
...
Examples of outputs of running this program are given in Fig. \ref{fir}: various
test FIRs are designed by selecting all coefficients except the first ones, which might be
either set to 1 for the first 16 coefficients, or to 4 for the first 4 coefficients (keeping
the total power constant). Since sampling is at 125~MS/s, the cutoff frequency
...
signal frequency becomes close to the cutoff frequency of the filter.
